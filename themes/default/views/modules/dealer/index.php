													...
													$('#countdown-clock-<?= $a->auction_id; ?>').countdown(end_auction_time)
														.on('update.countdown', function(event) {
															if(event.offset.totalSeconds < 2){
																...
																window.location.href = "<?= site_url('dealer/index'); ?>";
															}else{
																var format = '%H:%M:%S';
						...
						<div class="row">
							<div class="columns small-12 text-center">
								<ul class="pagination" role="navigation" aria-label="Pagination">
									...
									<li class="pagination-previous <?= $disabled ?>"><a href="<?= site_url('dealer/index/'.$first_page);?>"><i class="fa fa-angle-double-left"></i></a></li>
									<li class="<?= $disabled ?>"><a href="<?= site_url('dealer/index/'.$prev_page);?>"><i class="fa fa-angle-left"></i></a></li>
									
									<?php for ($i=1; $i<=$num_pages ; $i++) { 
											if($cur_page == $i) { $current = 'current'; } else { $current = ''; } ?>
										<li class="<?= $current ?>"><a href="<?= site_url('dealer/index/'.$i);?>"><?= $i ?></a></li>
									<?php } ?>
									
									<?php $next_page = (int) $cur_page + 1; $last_page = (int) $i - 1; 
										if( $next_page > $last_page) { $disabled = 'disabled'; $next_page = $last_page; } else { $disabled = ''; }?>
									<li class="pagination-next <?= $disabled ?>"><a href="<?= site_url('dealer/index/'.$next_page);?>"><i class="fa fa-angle-right"></i></a></a></li>
									<li class="pagination-next <?= $disabled ?>"><a href="<?= site_url('dealer/index/'.$last_page);?>"><i class="fa fa-angle-double-right"></i></a></a></li>
								</ul>
							</div>
						</div>
						<?php } ?>
				   ...
				   if(resp.status=='success'){
						...
						window.location.href = "<?= site_url('dealer/index'); ?>";
		  				...
		  				if(!found){
		  					...
		  					// $('.add-proxy-bid').closest('.proxy_value_container').find('.proxy_value').val("");
		  					window.location.href = "<?= site_url('dealer/index'); ?>";
		  				} else {
