				...
				<div class="container">
					<?php for($i=0; $i<count($chat); $i++) {
						if($chat[$i]->sender_id != $dealer_id) { ?>
							...
							<p class="timestamp" style="text-align: left;"><?= date('H:i:s',strtotime($chat[$i]->date_time)); ?></p>
						<?php } else { ?>
							<div class="item you">
								<p><?= $chat[$i]->content; ?></p>
							</div>
							<p class="timestamp" style="text-align: right;"><?= date('H:i:s',strtotime($chat[$i]->date_time)); ?></p>
						<?php }
					} ?>
				</div>
			</div>
